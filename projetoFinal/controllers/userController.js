...
const getUserTweets = async (req, res) => {
  try {
    ...
    const result = tweets.map((t) => {
      ...
      j.likedByMe = req.user
        ? j.Likes?.some((l) => l.userId === req.user.id)
        : false;
      delete j.Likes;
      delete j.Comments;
      return j;
    ...
    return res
      .status(500)
      .json({ message: "Erro ao obter tweets do utilizador." });
  }
};
...
const follow = async (req, res) => {
  try {
    ...
    if (!created)
      return res
        .status(409)
        .json({ message: "Já estás a seguir este utilizador." });
...
const unfollow = async (req, res) => {
  try {
    ...
    if (!deleted)
      return res
        .status(404)
        .json({ message: "Não estás a seguir este utilizador." });
